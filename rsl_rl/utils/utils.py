...
def resolve_callable(callable_or_name: type | Callable | str) -> Callable:
    ...
    for _, module_name, _ in pkgutil.iter_modules(rsl_rl.__path__, "rsl_rl."):
        try:
            ...
            # Some modules (e.g. packaging helpers like rsl_rl.setup in editable installs)
            # are not safe to import at runtime when resolving symbols by simple name.
            continue
        if hasattr(module, callable_or_name):
            return getattr(module, callable_or_name)

    # Check common rsl_rl namespaces directly (fast path for simple names like MLPModel, PPO, etc.)
    common_namespaces = [
        "rsl_rl.algorithms",
        "rsl_rl.models",
        "rsl_rl.runners",
        "rsl_rl.modules",
        "rsl_rl.distributions",
    ]
